...
import { FolderThumbnail } from '@/components/library/thumbnail/FolderThumbnail';
...
export const LibraryDragOverlayContents = ({ activeId, data }: LibraryDragOverlayProps) => {
  ...
  if (resource.resourcetype === 'VideoFile' || resource.resourcetype === 'ImageFile') {
    return <AssetThumbnail thumbnailUrl={resource?.thumbnail ?? ''} fileName={resource?.name} />;
  }
...
export const LibraryDragOverlay = ({ activeId, data }: LibraryDragOverlayProps) => {
  return (
    <AnimatePresence>
      {activeId ? (
