...
import { Sort, useLibraryStore } from "../../../../stores/library-store";
...
export const DateSortPill = ({ sort, order }: DateSortPillProps) => {
  ...
  const { sorts, setSorts } = useLibraryStore();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={() => setSorts(sorts.filter((s) => s.id !== sort.id))}
      >
        <CrossSmall size={20} />
      </div>
    </div>
