...
// import { useParams, useRouter } from 'next/navigation'; // Commented out
...
import { Listbox, ListboxItem, ListboxSection } from "../ui/Listbox"; // Fixed path

import { NewCanvasModal } from "./modals/canvas/NewCanvasModal"; // Fixed path
import { NewDocumentModal } from "./modals/documents/NewDocumentModal"; // Fixed path
import { NewVideoDraftModal } from "./modals/editor/NewVideoDraftModal"; // Fixed path
import { NewPhysicalAssetModal } from "./modals/physical-assets/NewPhysicalAssetModal"; // Fixed path
import { LibraryViewsPopoverContent } from "./popovers/LibraryViewsPopover"; // Fixed path

import { useFileUpload } from "../../hooks/useFileUpload"; // Corrected path

import { useAssetDetailsQuery } from "../../api-integration/queries/library"; // Corrected path

// import { useSearchActions, useSearchCommands } from '@/hooks/useSearch';

import { useLibraryStore } from "../../stores/library-store"; // Corrected path
import { useSearchActionsStore } from "../../stores/search-actions-store"; // Corrected path
import { useParamsStateStore } from "../../stores/params-state-store"; // Corrected path
...
export const WithLibraryThreeDotMenu = ({
  ...
  const { projectFolderId } = useParamsStateStore(); // Use store
  ...
  const { data: folderDetails } = useAssetDetailsQuery(projectFolderId ?? ""); // Use store value

  const hasFolderEditPermission =
    !projectFolderId ||
    (folderDetails && folderDetails?.permissions?.includes("can_edit_asset")); // Use store value
  ...
  const { uploadFile } = useFileUpload("library", projectFolderId || null); // Use store value
