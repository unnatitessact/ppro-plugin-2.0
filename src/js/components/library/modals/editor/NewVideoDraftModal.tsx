...
import { CreateVideoDraftSchema } from "@/schema/library/video-draft";
...
export const NewVideoDraftModal = ({
  isOpen,
  onOpenChange,
}: NewVideoDraftModalProps) => {
  // const { folderId: parentId } = useParams() as { folderId: string };

  const { folderId: parentId } = useParamsStateStore();
    ...
    <Modal isOpen={isOpen} size="sm" onOpenChange={onOpenChange}>
      <ModalContent
        ...
        onSubmit={handleSubmit(
          async (data) => await createVideoDraft(data.name)
        )}
      >
        <ModalHeader
