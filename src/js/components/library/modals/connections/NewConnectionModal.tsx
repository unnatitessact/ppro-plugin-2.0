...
export const NewConnectionModal = ({ isOpen, onOpenChange }: NewConnectionModalProps) => {
  const { mutateAsync, isPending } = useAddConnection();

  const createConnection = async (data: z.infer<typeof CreateConnectionSchema>) => {
    try {
      await mutateAsync(
        {
          name: data.name,
  ...
  return (
    <Modal isOpen={isOpen} size="sm" onOpenChange={onOpenChange}>
      <ModalContent as="form" onSubmit={handleSubmit((data) => createConnection(data))}>
        <ModalHeader
