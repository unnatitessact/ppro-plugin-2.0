    ...
    <div className="flex h-6 w-6 items-center justify-center">
      {state === "success" && (
        <CircleCheckFilled
          width={20}
          height={20}
          className="text-ds-toast-icon-success"
        />
      )}
      {state === "error" && (
        <CircleXFilled
          width={20}
          height={20}
          className="text-ds-toast-icon-error"
        />
      )}
      {state === "fallback" && (
        <TriangleExclamationFilled
          width={20}
          height={20}
          className="text-ds-toast-icon-warning"
        />
      )}
      {state === "process" && (
...
export const Loader = ({ visible }: { visible: boolean }) => {
  return (
    <div
      className="relative left-1/2 top-1/2 z-10 m-0 h-5 w-5"
