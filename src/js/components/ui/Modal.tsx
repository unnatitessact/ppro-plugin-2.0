...
const CustomModal = forwardRef<HTMLDivElement, NextUIModalProps>(
  (props, ref) => {
    return (
      <NextUIModal
        {...props}
  ...
  return (
    ...
    >
      {props.title || props.description ? (
        <>
          {props.subheading && (
            <p className="text-base font-medium text-ds-text-secondary">
              {props.subheading}
            </p>
          )}
          {props.title && (
            <h2 className="truncate text-2xl font-bold">{props.title}</h2>
          )}
          {props.description && (
            <p className="pr-8 text-sm font-normal text-ds-text-secondary">
              {props.description}
            </p>
          )}
        </>
