...
export const DateSortPill = ({ sort, order }: DateSortPillProps) => {
  ...
  const { removeSort, modifySort } = useLibraryFilterState();
  ...
  return (
    ...
    >
      <span className={cn('px-3 py-1', 'bg-ds-combo-pill-bg-label')}>{order}</span>
      <div className={nonClickablePillCn}>{sort.label}</div>
      <Dropdown>
        <DropdownTrigger>
          <div className={clickablePillCn}>{getLabelFromSort(sort.direction)}</div>
        </DropdownTrigger>
        <DropdownMenu
          onAction={(key) => modifySort(sort.id, { ...sort, direction: key as 'asc' | 'desc' })}
        >
