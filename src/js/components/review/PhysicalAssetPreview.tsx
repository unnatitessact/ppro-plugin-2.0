...
import { MetadataInfoItem } from "@/components/review/MetadataInfoItem";
...
export const PhysicalAssetPreview = ({ asset }: PhysicalAssetPreviewProps) => {
  ...
  const { mutate: addImageToPhysicalAsset, isPending } =
    useAddImageToPhysicalAsset(asset.id);
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-12 dark">
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          <MetadataInfoItem
            label="Added on"
            value={<p>{formatDateTime(asset.created_on)}</p>}
          />
          <MetadataInfoItem
            label="Added by"
            value={
              <User
                // @ts-ignore
                image={
                  (asset?.created_by?.profile.profile_picture as string) ?? ""
                }
                name={
                ...
                firstName={
                  asset?.created_by?.profile.first_name ??
                  asset?.external_user?.display_name ??
                  ""
                }
                lastName={asset?.created_by?.profile.last_name ?? ""}
                email={
                  asset?.created_by?.email ?? asset?.external_user?.email ?? ""
                }
                color={
          ...
          <MetadataInfoItem
            ...
            value={
              <User
                ...
                firstName={
                  asset?.modified_by?.profile.first_name ??
                  asset?.external_user?.display_name ??
                  ""
                }
                lastName={asset?.modified_by?.profile.last_name ?? ""}
                email={
                  asset?.modified_by?.email ?? asset?.external_user?.email ?? ""
                }
                color={
