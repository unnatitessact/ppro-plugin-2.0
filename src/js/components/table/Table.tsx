...
>({
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(
    () => {
      if (typeof window !== "undefined" && tableId) {
        ...
        return storedColumns.reduce((acc, col) => {
    ...
    if (typeof window !== "undefined" && tableId) {
      ...
      if (storedColumns.length === 0) {
        ...
        const orderedColumns = storedColumns
          .map((stored) =>
            filteredColumns.find((col) => col.key === stored.key)
          )
  ...
  useEffect(() => {
    ...
    if (localStorageColumns.length > 0) {
      const initialWidths = localStorageColumns.reduce((acc, column) => {
  ...
  useHotkeys("down", () =>
    setFocusIndex(Math.min(focusIndex + 1, data.length - 1))
  );
  useHotkeys("enter", () => {
    if (focusIndex >= 0 && focusIndex < data.length) {
      // onRowClick?.(data[focusIndex]);
      if (
        selectedData.filter((item: T) => item?.id === data[focusIndex]?.id)
          ?.length
      ) {
        setSelectedData(
          selectedData.filter((item: T) => item?.id !== data[focusIndex]?.id)
        ...
        {!hideColumnAddButton && (
          <Dropdown closeOnSelect={false}>
            <DropdownTrigger>
              <Button
                size="lg"
                className={`absolute right-0 top-0 z-50 h-11 rounded-none rounded-tr-xl bg-default-100 ${
                  hasTableHeader
